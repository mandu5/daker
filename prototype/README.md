...
사전 선언 게이트에서 기각된 가설(카복실산→신기능, lift 1.06 p=1.0e-02 — 층화 후
무너짐)이 조항을 발행시키면 시스템이 자기 증거 기준을 어기는 것이 된다.
...
다만 **봉인 사유를 뭉뚱그리면 안 된다.** 10개 조항 중 5개는 확증 기전이 없어
조항 후보가 **애초에 기안되지 않는다**(기전 게이트 상류). τ 보정 대상인 나머지
5개 중 2개(QT·CYP/DDI)가 발행 자격을 얻었고, 2개(음식·자몽 후보 395, 위장관 325)는
검정력이 충분한데 목표 미달 — 이것만이 진짜 "말할 자격 없음" 판정이다.
1개(간기능, 후보 9)는 확증 기전(하이드라진)이 희소해 **검정력 부족**이며,
정밀도가 낮다는 증거가 아니라 판정 보류다. `seal_breakdown` 으로 함께 보고한다.
