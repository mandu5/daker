...
def blocks():
    ...
    b += [
        ...
        {"type": "fine", "text":
            "연관이 곧 배포 가능한 판정 규칙은 아니다. 음식·자몽 조항은 연관이 가장 강한데"
            "(lift 1.91) 발행분 정밀도가 0이었다. 대신 사람검토 밴드에서 정밀도 0.182"
            "(기저 0.036 대비 5.1배)로 잡힌다. 신뢰도 임계 τ가 조항별로 보정되지 않았다는 "
            "뜻이며 본선 W2 과제로 명시한다."},
    ...
    b += [
        ...
        {"type": "table",
         ...
         "rows": [
             ...
             ["검토 물량 절감", "**실측**",
              "CYP·DDI에서 **같은 수의 실제 조항을 찾기까지 검토량 25% 절감** "
              "(커버리지 5% 기준 정밀도 0.234→0.314, 0.234/0.314=74.6%). "
              "__측정된 정밀도에서 직접 유도한 값이며 추정이 아니다__"],
             ["검토 시간", "~~추정~~",
