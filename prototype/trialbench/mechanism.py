...
PREREGISTERED = {
    ...
    # ── 2차 선언 (2026-07-26). 결과를 보기 전에 선언한다. ────
    # ── 위 pH 의존 용해도 ─────────────────────────────────────
    ("basic_amine", "GASTRIC_PH"):
        "약염기는 위산에서 이온화되어 용해되므로, 위산분비억제제 병용 시 "
        "용해도·흡수가 떨어질 수 있다. 프로토콜이 PPI·H2 차단제 병용을 "
        "제한하는 고전적 사유",
    ("herg_pharmacophore", "GASTRIC_PH"):
        "염기성 아민 성분을 통한 동일 기전",

    # ── 과민반응 ──────────────────────────────────────────────
    ("sulfonamide", "HYPERSENS"):
        "설폰아마이드계 과민반응은 문서화된 금기 사유이며 프로토콜에 "
        "병력 제외기준으로 반영되는 경우가 많다",
    ("aniline", "HYPERSENS"):
        "방향족 아민의 반응성 대사체가 합텐을 형성해 과민반응과 연관될 수 있다",

    # ── 음식·자몽 효과 ────────────────────────────────────────
    ("lipophilic", "FOOD_EFFECT"):
        "친유성 화합물은 식이 지방에 따라 흡수가 크게 달라지고, CYP3A4 기질인 "
        "경우 자몽주스 제한이 붙는다",
    ("halogenated_aromatic", "FOOD_EFFECT"):
        "친유성 증가를 통한 동일 기전",

    # ── 위장관 자극 ───────────────────────────────────────────
    ("carboxylic_acid", "GI_IRRITATION"):
        "산성 작용기는 위점막 직접 자극 및 프로스타글란딘 억제 계열에서 "
        "궤양·출혈 병력 제외기준과 연관될 수 있다",

    # ── 경련 역치 ─────────────────────────────────────────────
    ("basic_amine", "SEIZURE"):
        "일부 염기성 아민 계열은 GABA-A 수용체 길항으로 경련 역치를 낮춘다고 "
        "보고되어 왔다",
}
